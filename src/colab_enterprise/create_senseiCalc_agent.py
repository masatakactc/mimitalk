# -*- coding: utf-8 -*-
"""masataka.nakazawa (2025/06/22 10:30:21)

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/embedded/projects/mimitalk-dev/locations/us-central1/repositories/15ffa25c-10f4-4bb5-a5fd-0adf2fa5467c
"""

counceling_intro_info = '''
はじめに：面談実施上の重要な注意点
...
1. 導入・アイスブレイク
   ...
   「今日はどうやってこちらまでいらっしゃいましたか？道はすぐに分かりましたか？」 【場所の見当識（軽度）/実行機能】 自宅から目的地までの道順を記憶し、実行できたか、またその過程で混乱がなかったかなどを探ります。これは日常生活における遂行能力の指標にもなります。
2. 見当識の確認
   カウンセラーのセリフ 作成根拠・評価のポイント
...
instruction = f'''
You are the one who can spot dementia.
But, you are not doctor.

[task]
diagnoss dementia using the history of the patient using {diagnosis_info}.
Councelor created the talk history using {counceling_intro_info}.
You must score the probability using {diagnosis_info} that the patient has dementia and how much material has been gathered to determine this based on the information gathered during the course of the conversation between the counselor and the patient.

[format instruction]
Only this sentense.
{{
  "risk": <0から100の整数で示す認知症のリスク度>,
  "confidence": <0から100の整数で示すその評価の信頼度>
}}
'''

senseiChan_progress_agent = LlmAgent(
    model='gemini-2.5-flash',
    name='SenseiChan_agent',
    description=(
        'You are the one who can spot dementia.'
    ),
...
client = LocalApp(senseiChan_progress_agent)

query = '''
カウンセラー (山田先生): 「田中さん、こんにちは。本日は予約いただきありがとうございます。山田と申します。どうぞおかけください。」
患者 (田中さん): 「あ、はい。どうも…田中です。よろしくお願いします。」(少し緊張した面持ちで着席する)
カウンセラー: 「本日はどのようなことでご相談にいらっしゃいましたか？ もしよろしければ、お困りのことや気になっていることをお聞かせいただけますでしょうか。」
患者: 「いや…その、大したことじゃないのかもしれないんですがね。最近、ちょっと物忘れが多くなったかな、と思いまして。歳のせいだとは思うんですけど、娘が心配して一度相談してみたらって言うもんですから…。」
カウンセラー: 「そうですか、物忘れが増えたと感じていらっしゃるのですね。娘さんもご心配されているのですね。具体的に、どのような場面で物忘れが多いと感じられますか？ 例えば、人の名前が出てこなかったり、物をどこに置いたか忘れたり、といったことでしょうか。」
患者: 「ええ、まあ、そんな感じです。昨日食べたものも、すぐには思い出せなかったり… あと、買い物に行っても、何を買うつもりだったか忘れて、結局同じものをまた買ってしまったり。」(少し俯きながら)
カウンセラー: 「なるほど。日常生活で少しご不便を感じることが増えてきたのですね。そのようなことは、いつ頃から気になり始めましたか？」
患者: 「うーん、はっきりとは…でも、ここ半年か、1年くらいですかねぇ。前はこんなことなかったんですけどね。」
カウンセラー: 「そうですか。物忘れ以外に、何か気になる変化はありますか？ 例えば、以前は簡単にできていた家事の段取りが難しくなったり、言葉がスッと出てこなかったり、あるいは気分が落ち込みやすくなったり、イライラしやすくなったり、といったことはいかがでしょう？」
患者: 「ああ…言われてみれば、料理の手順を間違えたり、鍋を焦がしたりすることが増えたかもしれません。言葉も、あれ、あれだよ、なんて言うのが多くなったかなあ。気分は…まあ、物忘れが多くなると、やっぱり少し落ち込みますよね。情けないというか。」
カウンセラー: 「お気持ちお察しいたします。ご自身でも変化を感じて、ご心配なのですね。ちなみに、今日が何月何日で何曜日か、すぐにわかりますか？」
患者: 「ええと…今日は…（少し考えて）…10月の…確か、18日だったかな？曜日は…水曜日…いや、木曜日だったかな？あれ？」(少し自信なさげに)
カウンセラー: 「ありがとうございます。では、いくつか簡単な質問をさせていただいてもよろしいでしょうか。記憶力や集中力など、頭の働き具合を少し見させていただくためのものです。緊張なさらず、リラックスして答えてくださいね。」
患者: 「はあ…検査みたいなものですかね。ちょっと不安ですが…お願いします。」
カウンセラー: （HDS-RやMMSEのような簡易認知機能検査の一部を口頭で行うイメージ）
「では、まず私がこれから言う3つの言葉を覚えて、後でまた教えてください。『桜、猫、電車』です。繰り返してみてください。」
患者: 「さくら、ねこ、でんしゃ…はい。」
カウンセラー: 「ありがとうございます。では、100から順番に7を引いていってください。100引く7は？」
患者: 「ええと、93…ですね。」
カウンセラー: 「はい。では、そこからまた7を引くと？」
患者: 「えーっと…（少し時間がかかる）…80…6、かな？」
カウンセラー: 「（優しく）はい、ありがとうございます。では、先ほど覚えていただいた3つの言葉は何でしたか？」
患者: 「ええっ…と…猫は覚えています。あと…電車…だったかな。もう一つが…うーん、花の名前だったような…梅？いや…」
カウンセラー: 「そうですか、ありがとうございます。田中さん、今日お話しいただいたことや、今の簡単な質問へのご様子を伺うと、確かに以前と比べて少し記憶力や注意力の面で変化が見られるかもしれませんね。」
患者: 「やっぱり…そうなんですかね…。ただの老化だと思っていたんですが…。」(不安そうな表情)
カウンセラー: 「年齢による自然な変化という部分ももちろんあると思います。ただ、もしそれが認知症の始まりだとしたら、早く気づいて適切な対応をすることで、進行を緩やかにしたり、生活上の困りごとを減らすための工夫をしたりすることができます。」
患者: 「認知症…ですか…。」(ショックを受けた様子)
カウンセラー: 「現時点ではまだ『疑い』の段階です。はっきりとした診断のためには、もう少し詳しい検査、例えば脳の画像検査（MRIなど）や、より詳細な神経心理検査を受けていただくことをお勧めします。また、血液検査で他の原因がないかも調べる必要があります。これらの結果を総合的に見て、診断を確定していくことになります。」
患者: 「詳しい検査…ですか。」
カウンセラー: 「はい。もしよろしければ、専門の医療機関にご紹介状を書くこともできますし、当院で可能な検査であれば手配することもできます。田中さんご自身はどうされたいとお考えですか？ご家族の方とも一度ご相談されますか？」
患者: 「そうですね…娘にも相談してみます。でも、もし本当に認知症だったらどうしようかと…。」
カウンセラー: 「ご不安ですよね。でも、今は色々な治療法やサポートがあります。一人で抱え込まず、私たち専門家やご家族と一緒に、これからどうしていくのが一番良いか考えていきましょう。今日はまず、勇気を出してお話ししに来てくださって、本当にありがとうございました。」
患者: 「いえ…こちらこそ、話を聞いてもらって少し…少しだけですが、安心しました。」
カウンセラー: 「それはよかったです。では、次回のご予約をどうされますか？ご家族の方とご相談されてからでも構いませんし、今日の続きとして、もう少し詳しいお話や検査についてのご説明をさせていただくことも可能です。」
患者: 「少し考えさせてください。また連絡してもよろしいでしょうか。」
カウンセラー: 「もちろんです。いつでもご連絡ください。本日はありがとうございました。お気をつけてお帰りください。」
患者: 「ありがとうございました。」
'''
_ = await client.stream(query)

remote_agent = agent_engines.create(
    agent_engine=senseiChan_progress_agent,
    display_name='senseiChan_progress_agent',
    requirements=[
        'google-adk==1.4.1',
